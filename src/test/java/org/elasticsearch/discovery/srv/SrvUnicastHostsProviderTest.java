...
public class SrvUnicastHostsProviderTest {
    ...
    public void mockTransportService() throws Exception {
        ...
        for (int i = 0; i < 4; i++) {
            when(transportService.addressesFromString(eq(String.format(Locale.ENGLISH, "elastic%d.internal:44300",
                i + 1)), anyInt())).thenReturn(new TransportAddress[]{
                new LocalTransportAddress(String.format(Locale.ENGLISH, "elastic%d.internal", i + 1))
            });
        }
    }
    ...
    public void testBuildDynamicNoRecords() throws Exception {
        ...
        Settings.Builder builder = Settings.settingsBuilder()
            .put(SrvUnicastHostsProvider.DISCOVERY_SRV_QUERY, "_elastic._srv.elastic.internal");
    ...
    public void testBuildDynamicNodes() throws Exception {
        ...
        SrvUnicastHostsProvider unicastHostsProvider = new DummySrvUnicastHostsProvider(builder.build(),
            ...
            @Override
            protected Record[] lookupRecords() throws TextParseException {
                Name srvName = Name.fromConstantString("_elastic._srv.elastic.internal.");
                return new Record[]{
                    new SRVRecord(srvName, DClass.IN, 3600, 1, 10, 44300, Name.fromConstantString("elastic1.internal.")),
                    new SRVRecord(srvName, DClass.IN, 3600, 1, 20, 44300, Name.fromConstantString("elastic2.internal."))
                };
            }
        };
        List<DiscoveryNode> discoNodes = unicastHostsProvider.buildDynamicNodes();
    ...
    public void testParseRecords() throws Exception {
        ...
        Record[] records = new Record[]{
            new SRVRecord(srvName, DClass.IN, 3600, 1, 10, 44300, Name.fromConstantString("elastic1.internal.")),
            new SRVRecord(srvName, DClass.IN, 3600, 1, 20, 44300, Name.fromConstantString("elastic2.internal.")),
            new SRVRecord(srvName, DClass.IN, 3600, 2, 10, 44300, Name.fromConstantString("elastic3.internal.")),
            new SRVRecord(srvName, DClass.IN, 3600, 2, 20, 44300, Name.fromConstantString("elastic4.internal."))
        };
        ...
        assertEquals("#srv-elastic1.internal:44300", discoNodes.get(0).getId());
        assertEquals("#srv-elastic2.internal:44300", discoNodes.get(1).getId());
        assertEquals("#srv-elastic3.internal:44300", discoNodes.get(2).getId());
        assertEquals("#srv-elastic4.internal:44300", discoNodes.get(3).getId());
    }
}
